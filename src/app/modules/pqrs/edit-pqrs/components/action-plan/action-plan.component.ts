import { Component, OnInit, ViewChild, Input } from '@angular/core';
import { FormBuilder, FormGroup, Validators, FormArray, FormGroupDirective } from '@angular/forms';
import { MessageService } from 'src/app/shared/framework-ui/primeng/api/messageservice';
import { TranslateService } from '@ngx-translate/core';
import { PqrsService } from 'src/app/core/services/pqrs/pqrs.service';
import { fileUploadPqrs } from 'src/app/shared/models/fileUploadPqrs';
import { environment } from 'src/environments/environment';
import { DatePipe } from '@angular/common';
import { StorageService } from 'src/app/core/services/storage/storage.service';
import * as moment from 'moment'


@Component({
  selector: 'app-action-plan',
  templateUrl: './action-plan.component.html',
  styleUrls: ['./action-plan.component.css'],
  providers: [MessageService]
})

export class ActionPlanComponent implements OnInit {

  constructor(private readonly formBuilder: FormBuilder,
    public datepipe: DatePipe,
    public translate: TranslateService,
    private messageService: MessageService,
    private storageService: StorageService,
    private pqrsService: PqrsService) { }


  currentUserAplication: any;

  language = this.storageService.getLanguage();
  format: string = this.language == "en" ? 'MMM/DD/YYYY' : 'DD/MMM/YYYY';
  @Input() documentId: string
  @Input() isReadOnlyComponent: boolean = false;

  //menu

  formOneIconStatusOne = 0;
  formOneStatusTextOne = "";
  
  planAction;
  registerFormActionPlan: FormGroup;  

  // files
  acceptFileTypes: string;
  acceptDetailFileTypes: string;
  fileList: fileUploadPqrs[] = new Array<fileUploadPqrs>();
  file: any;

  // file validation error
  showNotFoundFile: boolean = false;
  hideErrorType: boolean;
  hideErrorSize: boolean;
  displayInvalidateFileMessage: boolean = false;
  maxSize: string;

  allowedFileTypes = [
    'image/png',
    'image/jpeg',
    'application/pdf',
    'image/bmp',
    'image/gif',
    'image/tiff',
    'image/vnd.adobe.photoshop',
    'application/x-photoshop',
    'application/octet-stream',
    'image/vnd.adobe.photoshop',
    'application/postscript',
    'video/mp4',
    'video/avi',
    'application/vnd.ms-excel',
    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  ];

  @ViewChild('ngForm') ngForm: FormGroupDirective;
  submitted: boolean;
  settingsReceptionDate = {
    minDate: new Date(2021, 1 - 1, 1),
    isRange: false,
    required: false,
    dateFormat: this.language == 'en' ? 'M/dd/yy' : 'dd/M/yy',
    ids: ['actionDate'],
    labels: 'pqrs.plan-action-date-action',
  };

  settingsSendDate = {
    minDate: new Date(2021, 1 - 1, 1),
    isRange: false,
    required: false,
    dateFormat: this.language == 'en' ? 'M/dd/yy' : 'dd/M/yy',
    ids: ['actionDate'],
    labels: 'pqrs.plan-action-date-action',
  };

  get aw() {
    return this.registerFormActionPlan.controls;
  }


  ngOnInit(): void {
    this._InitForms();
    this.currentUserAplication = this.storageService.getUser();
    this.getPlanActionByDocumenteId(this.documentId);

    this.validateActionPlan();
  }

  blockFormsregisterFormActionPlan()
  {        
    for (const ctrl in this.registerFormActionPlan.controls)     
      this.registerFormActionPlan.get(ctrl).disable();          
  }

  _InitForms() {
    this.registerFormActionPlan = this.formBuilder.group({
      description: ['', [Validators.nullValidator, Validators.maxLength(100)]],
      nameAttachment: ['', [Validators.nullValidator, Validators.maxLength(256)]],
      actionDate: ['', [Validators.nullValidator]],
      closingDate: ['', [Validators.nullValidator]],
      uploadEvidence: ['', [Validators.nullValidator]],
      urlAttachment: ['', Validators.nullValidator],
    });

  }

  validateActionPlan() {

    if(this.isReadOnlyComponent)
    {
      this.formOneIconStatusOne = 0;
      this.formOneStatusTextOne = "";
    }
    else
    {
      this.formOneIconStatusOne = 1
      this.formOneStatusTextOne = "technical-sheets.session_form_completed"
  
      this.registerFormActionPlan.valueChanges.subscribe(value => {
        this.formOneIconStatusOne = 1;
        this.formOneStatusTextOne = "technical-sheets.session_form_completed";
      });
    }    
  }

  getPlanActionByDocumenteId(numberRequest: string) {
    let data = {
      documentId: numberRequest
    };

    this.pqrsService.getActionPlanByDocument(data)
      .subscribe(
        (response) => {
          if (response) {
            this.planAction = response.data;
            if (this.planAction != null) {
              var actionDate = moment(this.planAction.actionDate).format(this.format);
              this.registerFormActionPlan.controls.actionDate.disable();
              this.registerFormActionPlan.controls.description.disable();
              this.registerFormActionPlan.controls.uploadEvidence.setValue(this.planAction.nameAttachment);
              this.registerFormActionPlan.controls.actionDate.setValue(actionDate);
              this.registerFormActionPlan.controls.description.setValue(this.planAction.description);
              this.registerFormActionPlan.controls.urlAttachment.setValue(this.planAction.urlAttachment);
            }
          }

        },
        (error) => {
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: error.message,
          });
        },
        () => { }
      );


    if(this.isReadOnlyComponent) 
    {      
      this.blockFormsregisterFormActionPlan();
      this.planAction = {};
    }
  }

  savePlanAction() {

    var actionDate = this.registerFormActionPlan.controls.actionDate.value;
    var description = this.registerFormActionPlan.controls.description.value;

    var _actionDate = this.registerFormActionPlan.get('actionDate').value ?
    moment(this.registerFormActionPlan.get('actionDate').value.split(' - ')[0], this.format)
    .format('YYYY-MM-DD') : null;
    

    if (this.planAction == null && (actionDate != '' || description != '' || this.fileList.length > 0)) 
    {
    
      var dataInsert =
      {
        documentId: this.documentId,
        description: this.registerFormActionPlan.controls.description.value,
        closingDate: _actionDate,
        actionDate: _actionDate,
        createdByUser: this.currentUserAplication.email,
        files: this.fileList
      }
      this.pqrsService.createPlanAction(dataInsert)
        .subscribe(
          (response) => {
            if (response) {

            }
            else {
              this.translate
                .stream('general.msgDetailResponse')
                .subscribe((res: string) => {
                  this.messageService.add({
                    severity: 'info',
                    summary: 'Info',
                    detail: res,
                  });
                });
            }
          },
          (error) => {
            this.messageService.add({
              severity: 'error',
              summary: 'Error',
              detail: error.message,
            });
          }
        );
    }    
  }

  // upload file logic
  public resetFiles() {
    this.aw.uploadEvidence.setValue('');
    this.fileList = new Array<fileUploadPqrs>();
  }

  async fileChanged(e, inputFile) {
    this.file = e.target.files[0];
    await this.upload(this.file, this.allowedFileTypes);
    inputFile.value = "";
  }

  async upload(fileContent: any, validator: string[]) {
    if (await this.validateUploadedFile(fileContent, validator)) {
      await this.convertFileToBase64(fileContent);
    }
  }

  async validateUploadedFile(fileContent: any, validator: string[]) {
    this.hideErrorType = true;
    this.hideErrorSize = true;
    this.displayInvalidateFileMessage = false;

    this.maxSize = environment.max_file_size_pqrs + 'MB'

    if (validator.indexOf(fileContent.type) == -1) {
      this.hideErrorType = false;
    }

    if (fileContent.size > (environment.max_file_size_pqrs * 1000000) ||
      fileContent.size == 0) {
      this.hideErrorSize = false;
    }
    this.displayInvalidateFileMessage = !this.hideErrorSize || !this.hideErrorType;
    return !this.displayInvalidateFileMessage;
  }

  async convertFileToBase64(file) {
    var self = this;
    const reader = new FileReader();
    reader.readAsDataURL(file);
    reader.onload = function () {
      let fileFU: fileUploadPqrs = {
        fileName: file.name,
        fileTemporal: reader.result.toString().split(',')[1]
      }

      self.fileList.push(fileFU);
      self.updateFileList();
    };
    reader.onerror = function (error) {
      console.log('Error: ', error);
    };
  }

  updateFileList() {

    let filesUpload: string = '';
    this.fileList.forEach((item, index) => {
      filesUpload += item.fileName + ' - ';
    });
    this.aw.uploadEvidence.setValue(filesUpload);
    console.log("this.fileList:", this.fileList);
  }

  dowload() 
  {      
    var urlAttachment = this.registerFormActionPlan.controls.urlAttachment.value;    
    if (urlAttachment == null || urlAttachment == '') {
      this.showNotFoundFile = true;
    }
    else{
      const byteCharacters = atob(this.registerFormActionPlan.controls.urlAttachment.value);
      const byteNumbers = new Array(byteCharacters.length);
      for (let i = 0; i < byteCharacters.length; i++) {
        byteNumbers[i] = byteCharacters.charCodeAt(i);
      }
      const byteArray = new Uint8Array(byteNumbers);
      const blob = new Blob([byteArray], { type: 'application/x-zip-compressed' });
      let url = URL.createObjectURL(blob);
      let a = document.createElement('a');
      document.body.appendChild(a);
      a.setAttribute('style', 'display: none');
      a.href = url;
      a.download = this.registerFormActionPlan.controls.nameAttachment.value;
      a.click();
      window.URL.revokeObjectURL(url);
      a.remove();
    }
  }

}
