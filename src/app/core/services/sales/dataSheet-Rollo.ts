...
import { DataSheetPiezas } from './dataSheet-piezas';
import { DataSheetLaser } from './dataSheet-laser';
...
export class DataSheetGetByRollo {

  constructor(
    private productService: ProductService,
    private translate: TranslateService,
    private storageService: StorageService,
    private dataSheetPiezas: DataSheetPiezas,
    private dataSheetLaser: DataSheetLaser
  ) {
  }

  async DatasheetByLine(data: any, sampleId: number) {
    ...
    this.productService.getSampleById(dataSampleById).subscribe(
      (response) => {
        ...
        this.buildPDFRoll(statusSample, data)
      },
      (error) => {
  ...
  buildPDFRoll(statusSample: string, data: any): void {
    switch (data.headerDto?.lineId) {
      case 52:
        this.rolloPDFReport(data, statusSample);
        break;

      case 84:
        this.dataSheetLaser.DatasheetByLineLaser(data, statusSample);
        break;

      case 56:
        this.dataSheetPiezas.DatasheetByLinePiezas(data, statusSample)
        break;
  ...
  private async rolloPDFReport(data: any, statusSample: string): Promise<void> {
    ...
    data.bodyDto.forEach((element) => {
      ...
      dataRow.push({
        text: element.middleHeat,
        alignment: 'center',
        fontSize: 6,
      });
      dataRow.push({
        text: element.inletHeat,
        alignment: 'center',
        fontSize: 6,
      });
      dataRow.push({
    ...
    pdf.add([
      ...
      new Table([
        [
          ...
          {
            text: this.translatorFinotex("pdf-report.screenOff"),
            style: 'tableHeader',
            alignment: 'center',
          },
          {
            text: data.headerDto.screenPeelOff,
            style: 'tableContent',
            alignment: 'center',
          },
          {
            text: this.translatorFinotex("pdf-report.offCont"),
            style: 'tableHeader',
            alignment: 'center',
          },
          {
            text: data.headerDto.offCont,
            style: 'tableContent',
            alignment: 'center',
          },
          {
        ...
        new Stack([
          new Table([
            ...
            [
              ...
              {
                text: this.translatorFinotex("pdf-report.heatingMiddle"),
                style: 'tableHeaderBody',
                fillColor: '#C8C9CB',
                alignment: 'center',
              },
              {
                text: this.translatorFinotex("pdf-report.heatingIntel"),
                style: 'tableHeaderBody',
                fillColor: '#C8C9CB',
                alignment: 'center',
              },
      ...
      ]).columnGap(3).margin([0, 10, 0, 0]).end,
      ...
      new Columns([
        new Stack([
          new Columns([
            ...
            //columna aplicada finotex
            new Stack([
              new Table([
                [
                  new Cell(
                    new Txt(this.translatorFinotex("pdf-report.appliedFtx")).bold().alignment('center').fontSize(7).end
                  ).fillColor("#C8C9CB").end,
                ],
                [
            ...
            new Stack([
              ...
              new Table([
                [
                  new Cell(
                    new Txt(this.translatorFinotex("pdf-report.winding")).bold().alignment('center').fontSize(7).end)
                    .fillColor("#C8C9CB").end,
                ],
                [
                  await new Img(data.footerDto.windingUrlImage == null ? Base64Imagen.NotImagen : data.footerDto.windingUrlImage)
                    .height(115)
                    .width(100).alignment('center')
                    .build(),
                ]
            ...
            new Stack([
              new Table([
                ...
                [
                  ...
                  new Cell(new Txt(data.footerDto.pressure).bold().alignment('center').fontSize(7).end).end,

                ],
                [
                  new Cell(new Txt(this.translatorFinotex("pdf-report.temperature")).bold().alignment('left').fontSize(7).end).end,

                  new Cell(new Txt(data.footerDto.temperature).bold().alignment('center').fontSize(7).end).end,

                ],
                [
                  new Cell(new Txt(this.translatorFinotex("pdf-report.time")).bold().alignment('left').fontSize(7).end).end,

                  new Cell(new Txt(data.footerDto.time).bold().alignment('center').fontSize(7).end).end,

                ],
                [
                  new Cell(new Txt(this.translatorFinotex("pdf-report.detachment")).bold().alignment('left').fontSize(7).end).end,

                  new Cell(new Txt(data.footerDto.datachment).bold().alignment('center').fontSize(7).end).end,
          ...
          ]).columnGap(6).margin([0, 25, 0, 0]).end,
      ...
      ]).columnGap(3).margin([0, (data.bodyDto.length > 0)? 220 : 15, 0, 0]).end,
