...
import { OverlayService, PrimeNGConfig, PrimeTemplate, SharedModule, TranslationKeys } from '../api/public_api';
import { ObjectUtils, UniqueComponentId, ZIndexUtils } from '../utils/public_api';
...
@Component({
    ...
    <span #container [ngClass]="{'p-calendar':true, 'p-calendar-w-btn': showIcon, 'p-calendar-timeonly': timeOnly, 'p-calendar-disabled':disabled, 'p-focus': focus}" [ngStyle]="style" [class]="styleClass">
        <ng-template [ngIf]="!inline">
            <input #inputfield type="text" [attr.id]="inputId" [attr.name]="name" [attr.required]="required" [attr.aria-required]="required" [value]="inputFieldValue" (focus)="onInputFocus($event)" (keydown)="onInputKeydown($event)" (click)="onInputClick()" (blur)="onInputBlur($event)" (focusout)="updateDate($event)"
                ...
                [ngClass]="'p-inputtext p-component p-calendar-ph'" autocomplete="off" [attr.aria-labelledby]="ariaLabelledBy"
                ...
                <div class="p-datepicker-group-container">
                    <div class="p-datepicker-group" *ngFor="let month of months; let i = index;">
                        <div class="principal">
                            ...
                            <div class="detalle">
                                <button type="button" (click)="switchToMonthView($event)" (keydown)="onContainerButtonKeydown($event)" *ngIf="currentView === 'date'" class="p-datepicker-month p-link mr-1" [disabled]="switchViewButtonDisabled()">
                                    {{getMonthName(month.month)}}
                                </button>
                                <button type="button" (click)="switchToYearView($event)" (keydown)="onContainerButtonKeydown($event)" *ngIf="currentView !== 'year'" class="p-datepicker-year p-link ml-1" [disabled]="switchViewButtonDisabled()">
                                    {{getYear(month)}}
                                </button>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" *ngFor="let weekDay of weekDays;let begin = first; let end = last">
                                            <span classe="title-week-day">{{weekDay}}</span>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
...
export class Calendar implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    @Output() changeInputFinotes: EventEmitter<any> = new EventEmitter();
    ...
    constructor(public el: ElementRef, public renderer: Renderer2, public cd: ChangeDetectorRef, private zone: NgZone, private config: PrimeNGConfig, public overlayService: OverlayService, private translate: TranslateService) { }
    ...
    updateDate(event: any) {
        this.changeInputFinotes.emit(event);
    }
